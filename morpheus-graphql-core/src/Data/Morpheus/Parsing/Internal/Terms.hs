...
setOf :: (FromList ValidationResult map k a, KeyOf k a) => Parser a -> Parser (map k a)
...
uniqTuple :: (FromList ValidationResult map k a, KeyOf k a) => Parser a -> Parser (map k a)
uniqTuple parser =
  label "Tuple" $
    parens
...
uniqTupleOpt ::
  ( FromList ValidationResult map k a,
    Empty (map k a),
    KeyOf k a
  ) =>
  Parser a ->
