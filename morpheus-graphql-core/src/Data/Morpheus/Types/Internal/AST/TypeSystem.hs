...
instance NameCollision Error SchemaDefinition where
...
instance NameCollision Error RootOperationTypeDefinition where
...
mkSchema :: (Monad m, MonadError Error m) => [TypeDefinition ANY s] -> m (Schema s)
mkSchema types =
  traverse3
    (popByKey types)
...
typeReference types rootOperation =
  popByKey types rootOperation
    >>= maybe
      (throwError $ "Unknown type " <> msgValidation (rootOperationTypeDefinitionName rootOperation) <> ".")
      (pure . Just)
...
instance NameCollision Error (TypeDefinition cat s) where
  nameCollision x =
    "There can Be only One TypeDefinition Named " <> msgValidation (typeName x) <> "."
