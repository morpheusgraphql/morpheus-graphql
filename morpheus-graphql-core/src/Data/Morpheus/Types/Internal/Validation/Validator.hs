...
import Data.Morpheus.Rendering.RenderGQL (RenderGQL (..), renderGQLText)
...
renderScope
  Scope
    ...
    renderSection
      "Scope"
      ( "referenced by type "
          <> renderGQLText currentTypeName
          <> " of kind "
          <> renderGQLText currentTypeKind
          <> " in field "
          <> renderGQLText fieldname
      )
