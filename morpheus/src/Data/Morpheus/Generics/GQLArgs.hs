...
import qualified Data.Morpheus.Generics.GQLInput as I
                                                ( GQLInput(..) )
...
class GQLArgs p where
    ...
    default decode :: ( Show p , Generic p, D.Data p , GDecode Arguments (Rep p) ) => Arguments -> Maybe p -> Validation p
    decode args _ = to <$> gDecode initialMeta args

    introspect :: Proxy p -> [(GQL__InputValue,GQLTypeLib -> GQLTypeLib)]
    default introspect :: (Show p,  Selectors (Rep p) GQL__InputValue , D.Typeable p) => Proxy p -> [(GQL__InputValue,GQLTypeLib -> GQLTypeLib)]
