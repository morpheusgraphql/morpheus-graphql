...
data Env = Env {database :: TVar Database, reqHeaders :: Headers}

newtype Web a = Web {runWeb :: ReaderT Env (ExceptT ErrorCode IO) a}
  deriving
    ( Functor,
      Applicative,
      Monad,
...
userResolver UserRow {userId = thisUserId, userFullName} =
  ...
  where
    ...
    followsResolver = do
      ...
      users <- fmap userTable getDB
      let userFolloweeIds = map followeeId . filter ((== thisUserId) . followerId) $ follows
          userFollowees = filter ((`elem` userFolloweeIds) . userId) users
