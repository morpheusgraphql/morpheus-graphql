...
depends on `GQLDirective.DIRECTIVE_LOCATIONS` API requires implementation of one of the following
visitor typeclass instances: `VisitType`, `VisitField`, `VisitEnum`.

The following example demonstrates the RemovePrefix definition directive that
removes prefixes from GQL objects and input objects.

```hs
data RemovePrefix = RemovePrefix {prefix :: Text}
  deriving (Generic, GQLType)
...
instance VisitType RemovePrefix where
  visitTypeName RemovePrefix {prefix} = T.drop (T.length prefix)
