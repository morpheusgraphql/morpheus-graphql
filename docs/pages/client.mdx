...
on last stem we should define Morpheus HTTP client with URL and custom headers.
now we can use it to fetch Hero.
...
However, the result is a stream instead of a single response. To print these results every time
we register a new user, we can use the function `forEach`.
