...
Let's say there is also another team that wants to use the `Posts` application as well,
but also needs to provide `Authors` information with it. the new application should
allow query of all existing `authors` and extend the post type with the field `author`.
...
The first thing to note is that the post type used in this app does not need to
...
be the same as the dependency of `Post` in `App/Posts.hs`. The reason for this
