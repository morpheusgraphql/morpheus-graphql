...
import           Data.Morpheus.Types.Resolver                   ((:->) (..), MUTATION, MonadResolver (..), QUERY,
                                                                 WithEffect (..))
...
type MResult = WithEffect Text Value
...
instance (Encoder a (KIND a) res, ArgumentsConstraint p) => Encoder (QUERY IO p :-> a) WRAPPER res where
...
instance (Encoder a (KIND a) MResult, ArgumentsConstraint p) => Encoder (MUTATION IO Text p :-> a) WRAPPER MResult where
  __encode (WithGQLKind (MutationResolver resolver)) selection'@(key', Selection {selectionArguments, selectionPosition}) = do
    args <- ExceptT $ pure $ decodeArguments selectionArguments
