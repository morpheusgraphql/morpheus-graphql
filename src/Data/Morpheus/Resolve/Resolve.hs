...
import           Data.Morpheus.Server.ClientRegister
                                                ( GQLState
                                                , publishUpdates
                                                )
...
import           Data.Morpheus.Types.Internal.AST.Operator
                                                ( Operator(..)
                                                , Operator'(..)
                                                )
...
schema queryRes mutationRes subscriptionRes =
  subscriptionSchema subscriptionRes $ mutationSchema mutationRes $ querySchema
    queryRes
...
requestBodyDecodeError aesonError' =
  "Bad Request. Could not decode Request body: " <> LB.pack aesonError'
...
resolveByteString
  ...
  -> LB.ByteString
  -> IO LB.ByteString
resolveByteString rootResolver request = case eitherDecode request of
...
resolveStreamText
  ...
  => GQLRoot q m s
  -> InputAction Text
  -> IO (OutputAction Text)
resolveStreamText rootResolver (SocketInput id' request) =
  case eitherDecode $ toLBS request of
    Left aesonError' ->
      return $ NoEffect $ bsToText $ requestBodyDecodeError aesonError'
    Right req ->
      fmap encodeToText <$> resolveStream rootResolver (SocketInput id' req)
...
resolve rootResolver request = do
 ...
 where
  _resolve = do
    ...
    case rootGQL of
      Query operator' ->
        encodeQuery gqlSchema queryRes $ operatorSelection operator'
      Mutation operator' ->
        resultValue <$> encodeMutation mutationRes (operatorSelection operator')
      Subscription operator' -> resultValue
        <$> encodeSubscription subscriptionRes (operatorSelection operator')
   where
...
resolveStream rootResolver (SocketInput id' request) = do
 ...
 where
  _resolve =
    (ExceptT $ pure (parseGQL request >>= validateRequest gqlSchema))
      >>= resolveOperator
   where
    ...
    resolveOperator (Mutation operator') = do
      WithEffect channels value <- encodeMutation mutationRes
        $ operatorSelection operator'
      return PublishMutation
        { mutationChannels     = channels
        , mutationResponse     = value
        , subscriptionResolver = sRes
        }
     where
    ...
    resolveOperator (Subscription operator') = do
      WithEffect channels _ <- encodeSubscription subscriptionRes
        $ operatorSelection operator'
...
packStream
  :: GQLState
  -> (InputAction Text -> IO (OutputAction Text))
  -> LB.ByteString
  -> IO LB.ByteString
packStream state streamAPI request = do
  ...
  case value of
    PublishMutation { mutationChannels = channels, mutationResponse = res', subscriptionResolver = resolver' }
      -> do
