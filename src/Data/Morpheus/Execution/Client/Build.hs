...
defineType derivings TypeD {tName, tCons} =
  DataD [] (mkName tName) [] Nothing (map cons tCons) $
  map derive (["Show", "Generic"] ++ derivings)
  where
    ...
    cons ConsD {cName, cFields} = RecC (mkName cName) (map genField cFields)
      where
        genField FieldD {fieldNameD, fieldTypeD} =
          (mkName fieldNameD, defBang, genFieldT fieldTypeD)
          where
...
defineQuery QueryD {queryTypes = rootType:subTypes, queryText, queryArgTypes} = do
  rootDecs <-
    defineOperationType (queryArgumentType queryArgTypes) queryText rootType
